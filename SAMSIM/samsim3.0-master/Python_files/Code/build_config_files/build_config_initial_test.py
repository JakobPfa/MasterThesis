...
config['dt'] = 5  # time increment [s]]
config['time'] = 0.0  # initial value of time [s]
config['time_out'] = 3600  # time between outputs [s]
config['time_total'] = config['time_out'] * 24 * 10  # total length of simulation [s]
...
# **********************************************************************************************************************
config['thick_0'] = 0.005 #0.01 #0.01 so there are some more layers than needed!!
config['Nlayer'] = 90
...
# Set variable inputs
T_top = Tstep(-10, -5, config['time_total'], config['dt'])
#T_top = t_top_testcase_2(config['time_total'], config['timestep_data'])
