...
CONTAINS
  ...
  Subroutine  sub_heat_fluxes()
    ...
    USE mo_functions
    IMPLICIT NONE
    REAL(wp) ::T_old,emi, pen, temp, temp1, temp2
       ...
       IF (boundflux_flag==2) THEN
          ...
          IF (abs(time - time_input(time_counter)) < tolerance) THEN
             ...
             fl_sen   = fl_sen_input(time_counter)
             fl_lat   = fl_lat_input(time_counter)
          ELSE
             ...
             fl_sen  = (1._wp-temp)*fl_sen_input(time_counter-1)   +temp*fl_sen_input(time_counter)
             fl_lat  = (1._wp-temp)*fl_lat_input(time_counter-1)   +temp*fl_lat_input(time_counter)
          END IF
          ...
          !has to be calculated from the radiation imbalance.
             ...
             IF (thick_snow>=thick_min) THEN 
                fl_Q_snow = temp1
                CALL sub_fl_Q_snow(m_snow,thick_snow,T_snow,psi_s(1),psi_l(1),thick(1),T(1),fl_Q(1))
...
!           !If T_top below freezing
       ...
       IF (boundflux_flag==3) THEN
          ...
          IF (lab_snow_flag==0 .OR. thick_snow<=thick_min/100._wp) THEN
             ...
             IF (abs(thick_snow) < tolerance .AND. lab_snow_flag==1 .AND. styropor_flag == 1) THEN
