...
path_snow = path_runs + 'precip/run_MOSAiC_FYI_realsnow'
path_double_snow = path_runs + 'precip/run_MOSAiC_FYI_double_realsnow'
path_half_snow = path_runs + 'precip/run_MOSAiC_FYI_half_realsnow'
path_no_snow = path_runs + 'precip/run_MOSAiC_FYI_no_realsnow'
path_four_snow = path_runs + 'precip/run_MOSAiC_FYI_four_realsnow'

path_snow_fbs1 = path_runs + 'precip/run_MOSAiC_FYI_realsnow_fbs_1'
path_double_snow_fbs1 = path_runs + 'precip/run_MOSAiC_FYI_double_realsnow_fbs_1'
path_half_snow_fbs1 = path_runs + 'precip/run_MOSAiC_FYI_half_realsnow_fbs_1'

paths = {'real': path_snow, 'double': path_double_snow, 'half': path_half_snow, 'no': path_no_snow, 
         'four': path_four_snow, 'real_fbs': path_snow_fbs1, 'double_fbs': path_double_snow_fbs1, 'half_fbs': path_half_snow_fbs1}
...
for run in runs_fbs1: plt.plot(S_bu_mean_1[run], label = plot_labels[run])
plt.legend()
plt.xlabel('time [days]')
...
# plot weighted mean bulksalinity
for run in runs_fbs0: plt.plot(S_bu_mean[run][:t_thick_max[run],], label = plot_labels[run])
plt.legend()
plt.xlabel('time [days]')
plt.ylabel('mean salinity [psu]')
plt.title('Mean bulk salinities, weighted by layer thickness')
...
for run in runs_fbs1:    
    plt.plot(dat[run]['T'][t_thick_max[run],:],
             dat[run]['depth'][t_thick_max[run],:]/dat[run]['vital_signs'][t_thick_max[run],3] - 
             dat[run]['depth'][t_thick_max[run],1]/dat[run]['vital_signs'][t_thick_max[run],3], 
...
for run in runs_fbs1:    
    plt.plot(dat[run]['S'][228,1:],
             dat[run]['depth'][228,1:]/(dat[run]['vital_signs'][228,3])-dat[run]['depth'][228,1]/(dat[run]['vital_signs'][228,3]), 
             label = plot_labels[run])
plt.legend()
plt.xlabel('Salinity [g/kg]')
plt.ylabel('depth realtive to maximal ice thickness')
