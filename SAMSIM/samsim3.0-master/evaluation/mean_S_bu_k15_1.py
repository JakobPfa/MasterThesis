...
plt.xlabel('time [days]')
plt.ylabel('temperature [°C]')
plt.title('longwave flux forcing')
plt.show()

plt.plot(fl_sw)
plt.xlabel('time [days]')
plt.ylabel('temperature [°C]')
plt.title('shortwave flux forcing')
plt.show()


T2m_t = np.linspace(0,len(T2m), len(T2m))
def seasonalforcing(x,a,b,c,d):
    f = a * np.sin(b*x + c) + d
    return f
popt, pcov =  curve_fit(seasonalforcing, T2m_t, T2m, p0 = (10, 1e-5,200000, -20))
fig, ax = plt.subplots()
divider = make_axes_locatable(ax)
#cax = divider.append_axes('right', size='5%', pad=0.05)
ax.plot(T2m_t, seasonalforcing(T2m_t, *popt), 
         color = 'red', linestyle = '--')
ax.plot(T2m_t, T2m)
'''
...
ax.set_ylim(-2,0.15)
for run in runs_fbs1[1:]:
    ax.vlines(S_bu_mean[run][t_plot], -2,0.2,color = colormap(norm(k_values[run])), linestyle = 'dashed', alpha = 0.8)
ax.set_xlabel('Salinity [g/kg]')
ax.set_ylabel('depth realtive to maximal ice thickness')
