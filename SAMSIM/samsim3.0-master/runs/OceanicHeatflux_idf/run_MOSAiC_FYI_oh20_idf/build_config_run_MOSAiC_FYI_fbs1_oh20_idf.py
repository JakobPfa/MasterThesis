...
const_inputs = {'T_bottom': -1.8, 'S_bu_bottom': 34,
                'fl_q_bottom': 20., 'precip_s':0., 'precip_l':0., 'fl_sen':0.}
...
# additionaly fit an idealized forcing to T2m, fl_lw adn fl_sw:
...
#import forcings, already shortened while importing
input_data = ['fl_lw', 'fl_sw', 'T2m', 'precip_l']
dat_input = {}
for i in input_data:
    dat_input[i] = np.loadtxt(wd + '/input/' + i + '.txt')[int(t_del):]

#x/time axis for fitted forcings 
...
dat_input['T2m'] = seasonalforcing(time_input, *popt1)
dat_input['fl_lw'] = seasonalforcing(time_input, *popt2)
dat_input['fl_sw'] = shortwave_forcing(time_input, *popt3)

for i in input_data:
    np.savetxt(path_input + i + '.txt', dat_input[i])
