...
input_data = {'fl_lw', 'fl_sw', 'fl_sen', 'fl_lat', 'T2m'}
for input in list(input_data):
    data = np.loadtxt(wd + '/input/' + input + '.txt')
    np.savetxt(path_input + input + '.txt', data[int(t_del-1):])
    
# modify precip_l (snow driver) to half the precipitation
precip_l = np.loadtxt(wd + '/input/precip_l.txt')
precip_l = precip_l * 2.
np.savetxt(path_input + 'precip_l.txt', precip_l[int(t_del-1):])
