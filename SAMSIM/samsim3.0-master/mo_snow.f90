...
CONTAINS 
  ...
  SUBROUTINE snow_coupling(H_abs_snow, phi_s, T_snow, H_abs, H, phi, T, m_snow, S_abs_snow, m, S_bu)
    ...
    T_in = T
    CALL getT(H, S_bu, T_in, T, phi, 5702)
  ...
  SUBROUTINE snow_precip (m_snow, H_abs_snow, thick_snow, dt, liquid_precip_in, T2m, solid_precip_in)
  ...
  !>
  !! Subroutine for calculating snow erosion based on observational data (caused e.g. by drifting snow)
  !!
  !! Feeds with negative values in the snow precipitation input. Only when snow cover is present.
  !! Subtracts eroded snow from the snow layer. 
  !!
  !! @par Revision History
  !! Implemented by Niels Fuchs, UHH (2022-04-09)

  SUBROUTINE snow_erosion (m_snow, H_abs_snow, thick_snow, dt, liquid_precip_in, T2m, solid_precip_in)
    REAL(wp), INTENT(inout)        :: m_snow, H_abs_snow, thick_snow
    REAL(wp), INTENT(in)           :: liquid_precip_in, T2m, dt
    REAL(wp), INTENT(in), OPTIONAL :: solid_precip_in
    REAL(wp)                       :: solid_precip, liquid_precip, d_thick
  ...
  SUBROUTINE snow_precip_0 (H_abs, S_abs, m, T, dt, liquid_precip_in, T2m, solid_precip_in)
    ...
    REAL(wp), INTENT(in)           :: liquid_precip_in, T2m, dt
    REAL(wp), INTENT(in), OPTIONAL :: solid_precip_in
    REAL(wp)                       :: solid_precip, liquid_precip
  ...
  !!
  SUBROUTINE sub_fl_Q_snow(m_snow, thick_snow, T_snow, psi_s_2, psi_l_2, thick_2, T_2, fl_Q)


    REAL(wp),  INTENT(in)  :: m_snow
    REAL(wp),  INTENT(in)  :: psi_s_2, psi_l_2
